...
// Copyright (c) 2013-2014 The Franko developers
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (auxpow != NULL) {
        ...
        // print to log
        LogPrintf("FrankoMiner: AUX proof-of-work found; our hash: %s ; parent hash: %s ; target: %s\n",
               hash.GetHex().c_str(),
    ...
    else
    {
        ...
        // print to log
        LogPrintf("FrankoMiner: proof-of-work found; hash: %s ; target: %s\n", hash.GetHex().c_str(), hashTarget.GetHex().c_str());
    }
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("FrankoMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FrankoMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FrankoMiner(CWallet *pwallet)
{
    LogPrintf("FrankoMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("franko-miner");
    ...
    try { while (true) {
        ...
        LogPrintf("Running FrankoMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("FrankoMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FrankoMiner, pwallet));
}
