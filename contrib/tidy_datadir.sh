...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Franko database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Franko datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Franko datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Franko 0.7 datadir."
    ;;
  3)
    echo "Detected Franko pre-0.8 datadir."
    ;;
  4)
    echo "Detected Franko 0.8 datadir."
    ;;
esac
